...
def cmd_fate(odds, cf, mode=None):
    rule = (mode == "rule")
    ...
    print(f"   ANSWER: {ans}")
    if ev: print(f"   ⚡ RANDOM EVENT (doubles, digit {digit} ≤ CF {eff_cf}) — resolve via oracle.py event-focus")
...
def cmd_scene(cf, mode="pure"):
    r = d(10)
    if r > cf:
        out = "EXPECTED SCENE (runs as framed)"
    elif r % 2 == 1:  # odd
        out = "ALTERED SCENE" if mode != "prepared" else "EXPECTED + added Random Event (prepared mode)"
    else:             # even
        if mode == "crafter": out = "INTERRUPT SCENE → generate an Adventure Crafter Turning Point"
        elif mode == "prepared": out = "EXPECTED + added Random Event (prepared mode)"
        else: out = "INTERRUPT SCENE → generate a Random Event (oracle.py event-focus)"
    print(f"🎬 SCENE TEST  [Chaos {cf} · mode {mode}]")
...
def main():
    ...
    try:
        if cmd == "fate": cmd_fate(pos[0], int(pos[1]), mode)
        elif cmd == "check": cmd_check(pos[0], int(pos[1]))
        elif cmd == "scene": cmd_scene(int(pos[0]), mode or "pure")
