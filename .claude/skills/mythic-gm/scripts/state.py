...
Commands:
  ...
  render <campaign>                Regenerate the Threads/Characters List bodies in campaign-state.md
                                   FROM the JSON (between the BEGIN/END markers) — the markdown Lists
                                   are a generated view, never hand-synced (kills dual-source drift).
...
RENDER_LABEL = {"thread": "THREADS", "character": "CHARACTERS"}
def _gen_list_md(campaign, kind):
    """Generate the Markdown body of a List from its JSON (name + optional note + weight).
    Characters group under their optional `group`; threads render as a numbered, weighted list."""
    obj = lists.load_list(campaign, kind)
    if not obj["entries"]: return f"_({kind.capitalize()}s List empty — see {lists.KIND_FILE[kind]})_"
    def line(e, bullet):
        note = (" — " + e["note"]) if e.get("note") else ""
        return f"{bullet} **{e['name']}**{note}  _(weight {e.get('weight',1)})_"
    if kind == "thread":
        return "\n".join(line(e, f"{i}.") for i, e in enumerate(obj["entries"], 1))
    groups = OrderedDict()
    for e in obj["entries"]:
        groups.setdefault(e.get("group") or "Roster", []).append(e)
    out = []
    for g, es in groups.items():
        out.append(f"**{g}**")
        out += [line(e, "-") for e in es]
    return "\n".join(out)

def cmd_render(campaign):
    path = os.path.join(campaign, "campaign-state.md")
    if not os.path.exists(path): sys.exit(f"No campaign-state.md in {campaign}")
    txt = open(path, encoding="utf-8").read(); done, skipped = [], []
    for kind, label in RENDER_LABEL.items():
        begin = f"<!-- BEGIN {label} (generated by `state.py render`) -->"
        end   = f"<!-- END {label} -->"
        block = begin + "\n" + _gen_list_md(campaign, kind) + "\n" + end
        pat = re.escape(begin) + r".*?" + re.escape(end)
        if re.search(pat, txt, re.DOTALL):
            txt = re.sub(pat, lambda m: block, txt, flags=re.DOTALL); done.append(label)
        else: skipped.append(label)
    open(path, "w", encoding="utf-8").write(txt)
    print(f"Rendered {', '.join(done) or 'nothing'} from JSON into {path}.")
    for s in skipped:
        print(f"  ⚠ no markers for {s} — add this pair where the list should generate:\n"
              f"     <!-- BEGIN {s} (generated by `state.py render`) -->\n     <!-- END {s} -->")
