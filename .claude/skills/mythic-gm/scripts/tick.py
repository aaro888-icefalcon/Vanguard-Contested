...
full bookkeeping checklist, so the recurring duties (world-tick, Chaos, List sync, seeds, and
the resolve-debt check) can't silently decay across a long, summarized session.
...
def world_tick_imperative(bdir):
    """The companion's world-tick OPERATIVE imperative — the same <!-- OPERATIVE:world-tick -->
    block bridge.py surfaces — so the checklist carries the RULE, not just the table rows."""
    try:
        import bridge as bridgemod
        return bridgemod.extract_operative(os.path.join(bdir, "subsystems.md"), "world-tick")
    except Exception:
        return None
...
def main():
    ...
    imp = world_tick_imperative(bdir)

    print(f"🧾 END-OF-SCENE BOOKKEEPING — scene {scene}  (complete EVERY item; do not skip)")
    print("  1. WORLD-TICK — advance each by rolling its named table (dice.py table <path>) / ticking its clock:")
    ...
    if imp:
        print("     — the world-tick imperative:")
        for ln in imp.splitlines():
            print(f"       » {ln}")
    print("  2. CHAOS FACTOR — judge honestly (unsure → +1):  state.py chaos <+1|-1> <CF>")
    print("  3. LISTS — update the JSON, then regenerate the snapshot:")
    print(f"       state.py thread|char add|weight|remove {camp} \"<name>\"   then   state.py render {camp}")
    print("  4. SEEDS — refresh the deck to 30–40 (canon + live world + generator rolls).")
    print("  5. RESOLVE-DEBT — did any PC task / skill / power / contested-social get a Fate Question")
    print("       instead of the RPG's own check this scene? If so, that was rung-1 leakage — correct it.")
