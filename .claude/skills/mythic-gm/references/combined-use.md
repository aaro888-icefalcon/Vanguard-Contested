...
Default = **Mythic 2e** rules (they supersede the older Adventure Crafter book guidance). **Plotline = Thread.** Use Mythic 2e Lists. The PC is *not* on the Characters List but may be Chosen on a blank.

## Where the Adventure Crafter plugs in
- **First Scene:** generate 1–3 Turning Points → seed Threads (Plotlines) & Characters.
...
| Move Toward / Move Away / Close A Thread | Threads List | (Close = Plotline **Conclusion** → end & remove the Thread) |
...
- **Mythic Random Event** (`oracle.py list <filled>`): die scales to fullness → usually hits an entry; a blank → **Choose Most Logical** or roll again.
...
If an Altered/Interrupt Turning Point yields **Conclusion** (1–8) on the Plot Points Table, the invoked Plotline/Thread ends this scene — engineer the ending and remove it from the List in Bookkeeping.
